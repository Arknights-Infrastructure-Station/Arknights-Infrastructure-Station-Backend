...
import com.arknightsinfrastructurestationbackend.dto.user.adminUser.AdminUserLoginResponse;
...
@RestController
@RequestMapping("/adminApi/lrf")
@AllArgsConstructor
@ExcludeFromTokenRefresh
public class AdminLRFController {
    ...
    public ResponseEntity<?> login(HttpServletRequest request, @RequestBody UserLRFData userLRFData) throws IllegalAccessException {
        userLRFData.setIpAddress(adminUserService.getUserIpFromRequest(request));
        String oldToken = Token.getTokenByRequest(request);
        OperateResult loginResult = adminUserService.login(userLRFData, oldToken);
        if (loginResult.isOk()) {
            AdminUserInfo adminUserInfo = adminUserService.getAdminUserInfo(loginResult.getResult(), true);
            ...
            // 创建包含管理员信息和newToken的响应体
            AdminUserLoginResponse adminUserLoginResponse = new AdminUserLoginResponse();
            adminUserLoginResponse.setMessage(loginResult.getMessage());
            adminUserLoginResponse.setAdminUserInfo(adminUserInfo);

            return ResponseEntity.ok().headers(headers).body(adminUserLoginResponse);
        } else {
    ...
    public ResponseEntity<OperateResult> registerUser(HttpServletRequest request, @RequestBody UserLRFData userLRFData) {
        userLRFData.setIpAddress(adminUserService.getUserIpFromRequest(request));
        try {
            OperateResult result = adminUserService.register(userLRFData);
    ...
    public ResponseEntity<OperateResult> forgetPassword(HttpServletRequest request, @RequestBody UserLRFData userLRFData) {
        userLRFData.setIpAddress(adminUserService.getUserIpFromRequest(request));
        try {
            OperateResult result = adminUserService.forgetPassword(userLRFData);
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new OperateResult(500, "内部服务器错误"));
