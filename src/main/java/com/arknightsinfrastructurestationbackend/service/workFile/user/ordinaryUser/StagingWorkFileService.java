...
/**
 * 普通用户专用的暂存作业服务类。
 * <p>提供用户鉴权、上传次数限制和权限校验，用户可插入、更新、删除和筛选自己的暂存作业。</p>
 */
@Slf4j
@Service
public class StagingWorkFileService extends BaseStagingWorkFileService {
    ...
    public StagingWorkFileService(StagingWorkFileMapper stagingWorkFileMapper,
                                  MowerBucketService mowerBucketService,
                                  CommonService commonService,
                                  AdapterService adapterService,
                                  UploadStagingWorkFileCountMapper uploadStagingWorkFileCountMapper,
                                  SelectUserService selectUserService) {
        super(stagingWorkFileMapper, mowerBucketService, commonService, adapterService);
        this.uploadStagingWorkFileCountMapper = uploadStagingWorkFileCountMapper;
        this.selectUserService = selectUserService;
    }

    /**
     * 为指定用户插入新的暂存作业记录。
     * <p>在插入前会进行用户验证、上传次数限制检查，并在成功插入后更新上传次数记录。</p>
     *
     * @param token           用户身份标识token
     * @param stagingWorkFile 要插入的暂存作业对象
     * @return 操作结果对象，表示插入操作的成功或失败状态
     * @throws ServiceException        业务逻辑处理异常时抛出（如上传次数更新失败）
    ...
    public OperateResult insertStagingWorkFile(String token, StagingWorkFile stagingWorkFile) throws ServiceException, JsonProcessingException {
        ...
        if (uploadStagingWorkFileCount != null && uploadStagingWorkFileCount.getCount() >= 50) { // 限制每天只能上传50份暂存作业文件
    ...
    /**
     * 根据用户token和简单筛选条件筛选暂存作业列表。
     * <p>会进行用户验证。若用户不存在则返回空列表。</p>
     *
     * @param token                用户身份标识token
     * @param workFileSimpleSearch 作业筛选简单参数
     * @return 符合条件的暂存作业列表，如果用户不存在则为空列表
     */
    ...
    public List<StagingWorkFile> screenStagingWorkFileList(String token, WorkFileSimpleSearch workFileSimpleSearch) {
        ...
        QueryWrapper<StagingWorkFile> queryWrapper = adapterService.createCommonUserLimitedQueryWrapper(workFileSimpleSearch, user.getId(), StagingWorkFile.class);
        return stagingWorkFileMapper.selectList(queryWrapper);
    }

    /**
     * 获取符合用户筛选条件的暂存作业总数。
     * <p>会进行用户验证。若用户不存在则返回0。</p>
     *
     * @param token                用户身份标识token
     * @param workFileSimpleSearch 作业筛选简单参数
     * @return 符合条件的暂存作业总数
    ...
    public Long getStagingWorkFileListCount(String token, WorkFileSimpleSearch workFileSimpleSearch) {
        ...
        QueryWrapper<StagingWorkFile> queryWrapper = adapterService.createCommonUserQueryWrapper(workFileSimpleSearch, user.getId(), StagingWorkFile.class);
