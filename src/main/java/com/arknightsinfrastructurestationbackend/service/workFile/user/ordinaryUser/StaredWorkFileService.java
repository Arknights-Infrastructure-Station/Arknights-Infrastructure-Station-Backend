...
/**
 * 普通用户专用的收藏记录服务类。
 * <p>提供添加、删除、获取用户收藏记录功能。</p>
 */
@Service
public class StaredWorkFileService extends BaseStarRecordService {
    ...
    public StaredWorkFileService(StarRecordMapper starRecordMapper,
                                 AdapterService adapterService,
                                 WorkFileMapper workFileMapper,
                                 RecyclingWorkFileMapper recyclingWorkFileMapper,
                                 SelectUserService selectUserService,
                                 CommonService commonService) {
        super(starRecordMapper, adapterService);
        this.workFileMapper = workFileMapper;
        this.recyclingWorkFileMapper = recyclingWorkFileMapper;
        this.selectUserService = selectUserService;
        this.commonService = commonService;
    }

    /**
     * 为当前用户插入新的收藏记录（收藏某个作业）。
     *
     * @param token 用户token
     * @param wid   作业文件ID
     * @return 操作结果对象，表示收藏操作的成功或失败状态
     */
    ...
    /**
     * 用户取消对某个作业的收藏记录。
     * <p>会进行用户验证和权限校验（只能取消自己收藏的作业）。</p>
     *
     * @param token 用户身份标识token
     * @param wid   工作文件ID
     * @return 操作结果对象，表示删除收藏操作的成功或失败状态
     */
    ...
    public OperateResult deleteStarRecord(String token, Long wid) {
        ...
        StarRecord existingRecord = getStarRecord(wid, user.getId());
        ...
        if (starRecordMapper.delete(new LambdaQueryWrapper<StarRecord>()
                ...
                .eq(StarRecord::getUid, user.getId())) > 0) {
    ...
    /**
     * 获取指定用户的收藏列表。
     * <p>需要用户验证。如果用户不存在则返回空列表。</p>
     *
     * @param token 用户身份标识token
     * @return 用户收藏的StarRecord列表
     */
    ...
    private List<Long> getUserStarredWorkIds(String token) {
        ...
        return getUserStarredWorkIds(user.getId());
    }
    ...
    private List<WorkFile> getUserStarredWorkFiles(WorkFileSimpleSearch workFileSimpleSearch, List<Long> workIds) {
        // 查询这些作业文件
        QueryWrapper<WorkFile> workFileQueryWrapper = adapterService.createCommonUserLimitedQueryWrapper(workFileSimpleSearch, null, WorkFile.class);
        ...
        QueryWrapper<RecyclingWorkFile> recyclingWorkFileQueryWrapper = adapterService.createCommonUserLimitedQueryWrapper(workFileSimpleSearch, null, RecyclingWorkFile.class);
